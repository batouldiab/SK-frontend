  ...
  useEffect(() => {
    const loadCsv = async () => {
      try {
        ...
        const response = await fetch("/data/green_fig7.csv");
        if (!response.ok) {
        ...
        // --- Global average (only across ENERGY green occupations) ---
        ...
        const postsCounts = topJobs.map((r) => r.postsCount);

        const totalPostsCount = postsCounts.reduce((sum, v) => sum + v, 0);
        ...
        const data = {
          ...
          datasets: [
            {
              type: "line",
              label: "Average green share (energy occupations)",
                ...
                label: function (context) {
                  ...
                  if (context.dataset.type === "bar") {
                    ...
                    return [
                      ...
                      `Posts count: ${posts.toLocaleString()}`,
                    ];
                  }
